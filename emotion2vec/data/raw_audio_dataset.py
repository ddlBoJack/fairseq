...
from fairseq.data import FairseqDataset, ConcatDataset
import bisect
...
class RawAudioDataset(FairseqDataset):
    ...
    def collater(self, samples):
        ...
        dataset_idxs = torch.LongTensor([s.get("dataset_idx", -1) for s in samples])
        if torch.any(dataset_idxs >= 0):
            input["dataset_idx"] = dataset_idxs
...
class Emotion2VecFileAudioDataset(RawAudioDataset):
    def __init__(
        self,
        manifest_path,
        sample_rate,
...
class Emotion2VecConcatDataset(FairseqDataset):
    @staticmethod
    ...
    def __getitem__(self, idx):
        ...
        sample_with_dataset_idx = self.datasets[dataset_idx][sample_idx]
        sample_with_dataset_idx["dataset_idx"] = dataset_idx
        return sample_with_dataset_idx
