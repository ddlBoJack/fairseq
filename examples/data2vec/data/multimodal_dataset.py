...
class MultimodalDataset(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        try:
            if hasattr(self.datasets[key], "getitem"):
                item = self.datasets[key].getitem(new_index)
            else:
    ...
    def __getitems__(self, indices):
        ...
        for index in indices:
            results.append(self.__getitem__(index))
        return results
    ...
    def collater(self, samples):
        ...
        if "full_id" in samples[0]:
            ...
            for sample in samples:
                ...
                dataset_idx_list.append(list(self.datasets.keys()).index(key))
                batch["net_input"]["dataset_idx"] = torch.Tensor(dataset_idx_list).long()
            return batch
        else:
